...
void ft_display_data_better() {
	float data_temperature_raw;
	float data_temperature_converted;
	char data_temperature_str[6];

	float data_humidity_raw;
	float data_humidity_converted;
	char data_humidity_str[6];

	data_humidity_raw = (g_data[1] << 12)
					| (g_data[2] << 4)
					| ((g_data[3] & 0b11110000) >> 4);

	data_temperature_raw = ((g_data[3] & 0b11110000) << 12)
					| (g_data[4] << 8)
					| (g_data[5]);
	...
	data_humidity_converted = (data_humidity_raw / 1048576UL) * 100;
	data_temperature_converted = (data_temperature_raw / 1048576UL) * 200 - 50;
	...
	// ft_uart_print_adc_10bit(data_temperature_converted);
	uart_printstr(data_temperature_str);
	uart_printstr(".C, Humidity: ");
	// ft_uart_print_adc_10bit(data_humidity_converted);
