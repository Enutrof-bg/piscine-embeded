...
uint32_t ft_average(uint32_t *array, uint8_t average) {
	return ((array[0] + array[1] + array[2]) / average);
}

void ft_shift_array(uint32_t *array, uint32_t value) {
	array[0] = array[1];
	array[1] = array[2];
...
void ft_display_data_better() {
	uint32_t data_temperature_raw;
	float data_temperature_converted;
	char data_temperature_str[10];
	static uint32_t data_temperature_average[3] = {0};
	static uint8_t average = 0;

	uint32_t data_humidity_raw;
	float data_humidity_converted;
	char data_humidity_str[10];
	static uint32_t data_humidity_average[3] = {0};


	data_humidity_raw = ((uint32_t)g_data[1] << 12)
					| ((uint32_t)g_data[2] << 4)
					| ((uint32_t)(g_data[3] & 0b11110000) >> 4);

	data_temperature_raw = ((uint32_t)(g_data[3] & 0b00001111) << 16)
					| ((uint32_t)g_data[4] << 8)
					| ((uint32_t)g_data[5]);
	

	ft_shift_array(data_humidity_average, data_humidity_raw);
	ft_shift_array(data_temperature_average, data_temperature_raw);
	...
	uart_printstr(data_temperature_str);
	uart_printstr(".C, Humidity: ");
	uart_printstr(data_humidity_str);
	uart_printstr("%");
	uart_printstr("\r\n");
}
